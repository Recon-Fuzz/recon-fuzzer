...
use crate::evm::cheatcodes::{CheatcodeInspector, CheatcodeState, HEVM_ADDRESS};
...
pub fn build_codehash_map(contracts: &[crate::evm::foundry::CompiledContract]) -> MetadataToCodehash {
...
pub struct DeploymentPcCounter {
    ...
    metadata_to_codehash: MetadataToCodehash,
...
impl DeploymentPcCounter {
    pub fn new(metadata_to_codehash: MetadataToCodehash) -> Self {
        Self {
    ...
    /// Set the metadata-to-codehash map (called after loading contracts)
    pub fn set_codehash_map(&mut self, map: MetadataToCodehash) {
        self.metadata_to_codehash = map;
    }

    /// Reset inspector for a new transaction (clears per-tx state)
    #[inline]
...
impl<CTX, INTR: InterpreterTypes> Inspector<CTX, INTR> for DeploymentPcCounter {
    fn step(&mut self, interp: &mut Interpreter<INTR>, _context: &mut CTX) {
        ...
        let codehash = if let Some(&create_codehash) = self.create_codehash_stack.last() {
            ...
            if let Some(&cached) = self.bytecode_ptr_cache.get(&ptr_key) {
                ...
                let codehash = resolve_codehash(&self.metadata_to_codehash, bytecode)
                .unwrap_or_else(|| {
    ...
    fn create(
        ...
        let codehash = resolve_codehash(&self.metadata_to_codehash, &init_code)
        .unwrap_or_else(|| {
...
pub struct CombinedInspector {
    ...
    /// Using codehash instead of address so same bytecode deployed to different addresses
    ///
    /// PERFORMANCE: Using Vec instead of HashSet for O(1) append during execution.
    /// Deduplication happens once at end of tx via `finalize_touched()`.
    /// This avoids expensive hash operations on every opcode step.
    ...
    /// Map from metadata hash to compile-time codehash (built at startup)
    pub metadata_to_codehash: MetadataToCodehash,
    /// Cheatcode state
...
impl CombinedInspector {
    pub fn new() -> Self {
        Self {
            ...
            bytecode_ptr_cache: HashMap::new(),
            metadata_to_codehash: HashMap::new(),
    ...
    /// Create with a metadata-to-codehash map
...
impl<CTX: ContextTr, INTR: InterpreterTypes> Inspector<CTX, INTR> for CombinedInspector {
    fn step(&mut self, interp: &mut Interpreter<INTR>, context: &mut CTX) {
        ...
        // CRITICAL FIX: Use call_depth (CALL frame count like Echidna's `length vm.frames`)
    ...
    fn call(&mut self, context: &mut CTX, inputs: &mut CallInputs) -> Option<CallOutcome> {
        ...
        if target == HEVM_ADDRESS {
            ...
            if input_data.len() >= 4 {
                use crate::evm::cheatcodes::{startPrankCall, stopPrankCall};
                ...
                use crate::evm::cheatcodes::dealCall;
                ...
                use crate::evm::cheatcodes::etchCall;
