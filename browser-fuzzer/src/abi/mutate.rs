...
pub fn mutate_abi_value<R: Rng>(rng: &mut R, value: &DynSolValue) -> DynSolValue {
    match value {
        ...
        DynSolValue::Bytes(b) => {
            DynSolValue::Bytes(super::mutator_array::mutate_ll(rng, None, vec![], b))
        }

        DynSolValue::String(s) => {
            let bytes = s.as_bytes().to_vec();
            let mutated = super::mutator_array::mutate_ll(rng, None, vec![], &bytes);
        ...
        DynSolValue::FixedBytes(b, size) => {
            ...
            let mutated = super::mutator_array::mutate_ll(rng, Some(*size), random_bytes, &bytes);
        ...
        DynSolValue::Array(elements) => {
            DynSolValue::Array(super::mutator_array::mutate_ll(rng, None, vec![], elements))
        }
        ...
        DynSolValue::FixedArray(elements) => {
            if elements.is_empty() {
                ...
                let mutated = super::mutator_array::mutate_ll(rng, Some(len), complement, elements);
...
fn generate_fresh_value<R: Rng>(rng: &mut R, element_type: &ElementType) -> DynSolValue {
    ...
    match element_type {
        ...
        ElementType::Uint(bits) => {
            let val = super::gen::gen_random_uint(rng, *bits);
            DynSolValue::Uint(val, *bits)
        }
        ElementType::Int(bits) => {
            let val = super::gen::gen_random_int(rng, *bits);
        ...
        ElementType::Tuple(len) => {
            ...
            let elements: Vec<DynSolValue> = (0..*len)
                .map(|_| DynSolValue::Uint(super::gen::gen_random_uint(rng, 256), 256))
                .collect();
            DynSolValue::Tuple(elements)
        }
        ...
        ElementType::Unknown => {
            // Fallback to uint256
            DynSolValue::Uint(super::gen::gen_random_uint(rng, 256), 256)
        }
    }
}
...
pub fn mutate_call<R: Rng>(
    ...
    // Mutate a single random argument
