...
use crate::evm::{INITIAL_BLOCK_NUMBER, INITIAL_TIMESTAMP};
use rand::Rng;
...
impl<T: Ord + Clone> CachedSet<T> {
    pub fn new() -> Self {
        Self { set: BTreeSet::new(), cache: Vec::new(), dirty: false }
    }

    pub fn insert(&mut self, value: T) -> bool {
        let inserted = self.set.insert(value);
        if inserted { self.dirty = true; }
        inserted
    }

    pub fn is_empty(&self) -> bool { self.set.is_empty() }
    pub fn len(&self) -> usize { self.set.len() }
    pub fn iter(&self) -> std::collections::btree_set::Iter<'_, T> { self.set.iter() }
    pub fn contains(&self, value: &T) -> bool { self.set.contains(value) }
...
impl<T: Ord + Clone> Extend<T> for CachedSet<T> {
    fn extend<I: IntoIterator<Item = T>>(&mut self, iter: I) {
        ...
        if self.set.len() != old_len { self.dirty = true; }
    }
}
...
pub struct GenDict {
    ...
    /// Uses CachedSet for O(1) random picks
...
impl GenDict {
    pub fn new(seed: u64) -> Self {
        ...
        // Add common small integers to help find bugs 
        ...
        // Add time constants (timeConstants in Solidity.hs)
    ...
    ///
    /// NOTE: In the browser-fuzzer build, this is a no-op since the `analysis` crate
    /// is not available. The full implementation lives in the main recon-fuzzer crate.
    pub fn seed_from_bytecode(&mut self, _bytecode: &[u8]) {
        // No-op: analysis crate not available in browser-fuzzer
    }

    /// Seed dictionary from multiple bytecodes
    pub fn seed_from_bytecodes(&mut self, _bytecodes: impl IntoIterator<Item = impl AsRef<[u8]>>) {
        // No-op: analysis crate not available in browser-fuzzer
    }
    ...
    pub fn mk_dict_values(values: &[DynSolValue]) -> CachedSet<U256> {
        let mut set = CachedSet::new();
        for val in values {
