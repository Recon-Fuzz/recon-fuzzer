...
/// Info extracted under read lock for expensive test checks done outside any lock
enum DeferredCheck {
    Property,
    Optimization,
}
...
pub fn check_tests_after_tx_worker(
    ...
    for test_ref in &env.test_refs {
        // Phase 1: Read lock to filter and classify
        let deferred = {
            ...
            match &test.test_type {
                TestType::CallTest { .. } | TestType::AssertionTest { .. } => None,
                TestType::PropertyTest { .. } => Some(DeferredCheck::Property),
                TestType::OptimizationTest { .. } => Some(DeferredCheck::Optimization),
                _ => continue,
            }
        }; // read lock dropped

        if let Some(deferred_kind) = deferred {
            // Phase 2: Expensive check with NO lock held
            let (val, res) = {
                let test = test_ref.read();
            ...
            // Phase 3: Write lock only to update state
        ...
        // Cheap tests (CallTest, AssertionTest): write lock is fine since check is fast
        {
            let mut test = test_ref.write();
            if !test.is_open() {
                continue;
...
pub fn check_tests_without_optimization_worker(
    ...
    for test_ref in &env.test_refs {
        ...
        let is_property = {
            ...
            match &test.test_type {
                TestType::PropertyTest { .. } => true,
                TestType::CallTest { .. } | TestType::AssertionTest { .. } => false,
        ...
        if is_property {
            // Phase 2: Expensive PropertyTest check with NO lock held
            let (val, res) = {
                let test = test_ref.read();
                let mut check_vm = vm.clone();
                check_etest(&mut check_vm, &test, sender)?
        ...
        // Cheap tests (CallTest, AssertionTest): write lock is fine
        {
            let mut test = test_ref.write();
            if !test.is_open() {
                continue;
